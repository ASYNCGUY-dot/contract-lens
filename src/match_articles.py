...
미탐·오탐이 0.50~0.51에서 교차한다. 이 서비스는 판정하지 않고 근거만 보여주므로
**미탐이 더 아프다** — 오탐은 사용자가 보고 넘기지만 미탐은 아예 안 보인다.
기본값을 0.43으로 둔다(실측상 미탐 1 / 오탐 20).
...
def cmd_demo():
    ...
    for title, body in samples:
        ...
        for r in res:
            print(f"   제{r['조']:>2}조 {r['제목']:12} 넓은 {r['넓은점수']:.4f} / 좁은 {r['좁은점수']:.4f}")
